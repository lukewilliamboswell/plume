...
to_html = |@Chart(inner)|
    ...
    title =
        ...
        when inner.layout is
            None -> default_title
            Some(layout) -> Layout.get_title(layout) |> Result.with_default(default_title)

    template
    |> Str.replace_first("{{CHART_JSON}}", to_json(@Chart(inner)))
    |> Str.replace_first("{{CHART_TITLE}}", title)
